...
async function showResult() {
    ...
    // Fake Patti Anim
    document.getElementById('patti-tiger').classList.add('patti-active');
    document.getElementById('patti-dragon').classList.add('patti-active');
    ...
    setTimeout(async () => {
        ...
        if(totalWin > 0) {
            ...
            Swal.fire("Winner!", "You Won: " + totalWin, "success");
        }

        setTimeout(() => {
            timeLeft = 15; canBet = true; myActiveBets = [];
            document.getElementById('patti-tiger').classList.remove('patti-active');
            document.getElementById('patti-dragon').classList.remove('patti-active');
            resDisplay.innerText = "Betting Open...";
            startTimer();
        }, 5000);
    }, 2000);
}
